...
//Text coloring
#define HEX(i) text_color = mod(vec3(i / 65536,i / 256,i),vec3(226.0))/255.0;
...
vec3 Text(vec2 uv)
{
	...
	HEX(0xFF0000) _ _ _K _I _L _L _ _Y _O _U _R _ _S _E _L _F _ _X _3  ;
	BEGIN_TEXT(res.x/2.0-STRWIDTH(11.0)/2.0,res.y/2.0)
	print_pos += vec2(cos(time)*96.,sin(time)*96.);

	// RGB(3,0,0) _M RGB(2,.5,0)_o RGB(2,1,0)_v RGB(0,1,0)_i RGB(0,.5,6)_n RGB(0.5,99,1)_g _ RGB(1,9,0)_T RGB(2,.5,0)_e RGB(1,98,0)_x RGB(0,1,0)_t
